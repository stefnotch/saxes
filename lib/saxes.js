...
const {
  XML_1_0: {
    ED5: { isS, isChar, isNameStartChar, isNameChar }
  }
} = require("xmlchars");
 ...
 * like.
 * @property {boolean} [trim] Should the text be trimmed?
 */
...
class SaxesParser {
  ...
  _init(opt) {
    ...
    this.reportedTextBeforeRoot = this.reportedTextAfterRoot = this.closedRoot = this.sawRoot = this.inRoot = fragmentOpt;
  ...
  fail(er) {
    const message = this.trackPosition
      ? `${this.fileName}:${this.line}:${this.column}: ${er}`
      : er;

    this.nodeStream.error(new Error(message));
    return this;
  }
  ...
  sDoctype() {
    ...
    if (c === GREATER) {
      this.state = S_TEXT;
      this.emitNode(EVENTS.doctype, this.doctype);
  ...
  sCommentEnding() {
    ...
    if (c === MINUS) {
      ...
      this.emitNode(EVENTS.comment, this.comment);
      this.comment = "";
    } else {
  ...
  sCDataEnding2() {
    ...
    switch (c) {
      case GREATER:
        this.emitNode(EVENTS.cdata, this.cdata);
        this.cdata = "";
        this.state = S_TEXT;
        break;
      case CLOSE_BRACKET:
        ...
        ? cx => {
            if (cx !== QUESTION && !isS(cx)) {
              if (!(check(cx) && cx !== COLON)) {
                this.fail(
                  "disallowed characer in processing instruction name."
                );
              }
      ...
      switch (this.xmlDeclState) {
        ...
        case S_XML_DECL_NAME:
          ...
          if (isS(c) || c === EQUAL) {
            ...
            this.xmlDeclState =
              c === EQUAL ? S_XML_DECL_VALUE_START : S_XML_DECL_EQ;
          }
          break;
        case S_XML_DECL_EQ:
          c = this.getCode();
...
/^[A-Za-z0-9][A-Za-z0-9._-]*$/."
  ...
  sPIEnding() {
    ...
    if (this.piIsXMLDecl) {
      ...
      this.emitNode(EVENTS.processinginstruction, {
  ...
  sOpenTag() {
    ...
    this.emitNode(EVENTS.opentagstart, tag);
  ...
  sAttribValueUnquoted() {
    const c = this.captureTo(ATTRIB_VALUE_UNQUOTED_TERMINATOR, "attribValue");
    if (c === AMP) {
      this.state = S_ENTITY;
      this.entityBufferName = "attribValue";
  ...
  sEntity() {
    ...
    if (c === SEMICOLON) {
      ...
      this[this.entityBufferName] += `&${this.entity}${String.fromCodePoint(
        c
      )}`;
      this.entity = "";
      this.state = this.entityReturnState;
   ...
   */
    ...
    this.closeText();
    this.closed = true;
    this.nodeStream.complete();
    this._init(this.opt);
    return this;
  }

  /**
   * If there's text to emit ``EVENTS.text``, emit it.
   *
   * @private
   */
  closeText() {
    if (this.textNode) {
      if(this.trim === true) {
        let txt = this.textNode.trim();
        if(txt != "") {
          this.nodeStream.next({type: EVENTS.text, data: txt });
        }
      } else {
        this.nodeStream.next({type: EVENTS.text, data: this.textNode });
      }
    }
    this.textNode = "";
    this.textNodeCheckedBefore = 0;
  ...
  emitNode(nodeType, data) {
    ...
    this.nodeStream.next({type: nodeType, data: data});
  }
  ...
  emitNodes(nodeTypeA, nodeTypeB, data) {
    ...
    this.nodeStream.next({type: nodeTypeA, data: data});
    this.nodeStream.next({type: nodeTypeB, data: data});
  }
  ...
  openTag(selfClosing) {
    ...
    const { tags } = this;
    if (selfClosing) {
      this.emitNodes(EVENTS.opentag, EVENTS.closetag, tag);
      const top = (this.tag = tags[tags.length - 1]);
      if (!top) {
        this.closedRoot = true;
      }
    } else {
      this.emitNode(EVENTS.opentag, tag);
      this.inRoot = true;
      tags.push(tag);
    }
    this.state = S_TEXT;
  ...
  closeTag() {
    ...
    while (l-- > 0) {
      ...
      this.emitNode(EVENTS.closetag, tag);
      if (tag.name !== name) {
